...
test("production registry applies one explicit precedence without conflicts", () => {
  ...
  assert.equal(PRODUCTION_THUMBNAIL_DECISIONS.size, 3560);
  assert.equal(GENERATED_PHASE5_REVIEWED_DECISION_RECORDS.length, 3456);
...
test("Phase 5 final review appends exactly 632 delegated approvals and preserves all prior Phase 5 decisions byte-for-byte", async () => {
  const source = await readFile("data/thumbnail-phase5-reviewed-decisions.csv", "utf8");
  const lines = source.split("\n");
  const preservedPrefix = `${lines.slice(0, 1279).join("\n")}\n`;
  assert.equal(
    crypto.createHash("sha256").update(preservedPrefix).digest("hex"),
    "9b3ce7aa8ae1972c7943a523651820df46e1e9b5632d830571d5ae987ed032ec",
  );
  const finalReview = GENERATED_PHASE5_REVIEWED_DECISION_RECORDS.filter(
    (record) => record.approval_batch === "phase5f-review-final",
  );
  assert.equal(finalReview.length, 632);
  assert.equal(new Set(finalReview.map((record) => record.code)).size, 632);
  assert.deepEqual(
    finalReview.reduce((counts, record) => {
      counts[record.mode] = (counts[record.mode] ?? 0) + 1;
      return counts;
    }, {}),
    { PACKAGE_RIGHT: 533, SAMPLE: 99 },
  );
  for (const record of finalReview) {
    assert.equal(record.approved_by, "owner_delegated_via_chatgpt", record.code);
    assert.equal(record.approval_batch, "phase5f-review-final", record.code);
    assert.match(record.reason, /owner delegated proxy approval via ChatGPT/, record.code);
    assert.match(record.reason, /not an auto-safe classification/, record.code);
    const effective = getProductionThumbnailDecision(record.code);
    assert.equal(effective?.approval_status, "HUMAN_APPROVED", record.code);
    ...
    if (record.mode === "SAMPLE") {
      ...
      assert.equal(record.mode, "PACKAGE_RIGHT", record.code);
      assert.equal(record.source_id, "dvd:right", record.code);
      assert.equal(record.output_path_or_url, `/card-thumbnails/${record.code}-auto-right.jpg`, record.code);
      const bytes = await readFile(`public${record.output_path_or_url}`);
      assert.equal(crypto.createHash("sha256").update(bytes).digest("hex"), record.output_hash, record.code);
...
test("Phase 5G exact-94 adaptive review appends only 84 delegated visual approvals", async () => {
  const source = await readFile("data/thumbnail-phase5-reviewed-decisions.csv", "utf8");
  const lines = source.split("\n");
  const preservedPrefix = `${lines.slice(0, 2063).join("\n")}\n`;
  assert.equal(
    crypto.createHash("sha256").update(preservedPrefix).digest("hex"),
    "91ef7c32f18a8245caedc08ddd866a78932ae45a3ac624456a8b8712ad60ee94",
  );
  const exact94 = GENERATED_PHASE5_REVIEWED_DECISION_RECORDS.filter(
    (record) => record.approval_batch === "phase5g-5274-6273",
  );
  assert.equal(exact94.length, 84);
  assert.equal(new Set(exact94.map((record) => record.code)).size, 84);
  assert.deepEqual(
    exact94.reduce((counts, record) => {
      counts[record.mode] = (counts[record.mode] ?? 0) + 1;
      return counts;
    }, {}),
    { PACKAGE_CENTER: 5, PACKAGE_RIGHT: 73, SAMPLE: 6 },
  );
  for (const record of exact94) {
    assert.equal(record.approved_by, "owner_delegated_via_chatgpt", record.code);
    assert.match(record.reason, new RegExp(record.code), record.code);
    assert.match(record.reason, /not an auto-safe classification/, record.code);
    const effective = getProductionThumbnailDecision(record.code);
...
test("Phase 5G 6567-7566 appends only the 19 Stage 3 visual approvals", () => {
  const batch = GENERATED_PHASE5_REVIEWED_DECISION_RECORDS.filter(
    (record) => record.approval_batch === "phase5g-6567-7566",
  );
  assert.equal(batch.length, 19);
  assert.equal(new Set(batch.map((record) => record.code)).size, 19);
  assert.deepEqual(
    batch.reduce((counts, record) => {
      counts[record.mode] = (counts[record.mode] ?? 0) + 1;
      return counts;
    }, {}),
    { PACKAGE_RIGHT: 4, SAMPLE: 15 },
  );
  const samples = {
    DVMM00417: "sample:1",
    BEBL00055: "sample:3",
    AQUMA00149: "sample:13",
    "1TLDC00057": "sample:1",
    "1SEVEN00035": "sample:1",
    "1NSBB00035": "sample:1",
    VRKM01838: "sample:1",
    URVRSP00580: "sample:2",
    "55TMAVR00283": "sample:1",
    SAVR01104: "sample:1",
    JUVR00298: "sample:1",
    CRNX00335: "sample:1",
    KAVR00501: "sample:1",
  ...
  const right = new Set(["ATID00685", "H_1711MAAN01179", "H_1651Y00404", "URVRSP00586"]);
  for (const record of batch) {
    assert.equal(record.approved_by, "owner_delegated_via_chatgpt", record.code);
    assert.match(record.reason, new RegExp(record.code), record.code);
    assert.match(record.reason, /not an auto-safe classification/, record.code);
    assert.equal(record.source_id, samples[record.code] ?? "dvd:right", record.code);
    assert.equal(right.has(record.code), record.mode === "PACKAGE_RIGHT", record.code);
    const effective = getProductionThumbnailDecision(record.code);
    assert.equal(effective?.approval_status, "HUMAN_APPROVED", record.code);
    assert.equal(effective?.render_status, "READY", record.code);
    assert.equal(effective?.mode, record.mode, record.code);
...
test("Phase 5G 7605-8604 appends the exact 530 delegated adaptive approvals", async () => {
  const source = await readFile("data/thumbnail-phase5-reviewed-decisions.csv", "utf8");
  const preservedPrefix = `${source.split("\n").slice(0, 2166).join("\n")}\n`;
  assert.equal(
    crypto.createHash("sha256").update(preservedPrefix).digest("hex"),
    "7eafee4d798a9aca18ec655e3a1b76afef76b01d8f44a80de1d39b2a2c27580f",
  );
  const batch = GENERATED_PHASE5_REVIEWED_DECISION_RECORDS.filter(
    (record) => record.approval_batch === "phase5g-7605-8604",
  );
  assert.equal(batch.length, 530);
  assert.equal(new Set(batch.map((record) => record.code)).size, 530);
  assert.deepEqual(
    batch.reduce((counts, record) => {
      counts[record.mode] = (counts[record.mode] ?? 0) + 1;
      return counts;
    }, {}),
    { PACKAGE_FULL: 10, PACKAGE_RIGHT: 427, SAMPLE: 93 },
  );
  const objectFit = {
    PACKAGE_FULL: "contain",
    PACKAGE_RIGHT: "cover",
    SAMPLE: "scale-down",
  };
  for (const record of batch) {
    ...
    for (const resolution of surfaces) {
      ...
      const contract = buildThumbnailRenderContract(resolution);
      assert.equal(contract.src, record.output_path_or_url, record.code);
      assert.equal(contract.object_fit, objectFit[record.mode], record.code);
      assert.equal(contract.object_position, record.mode === "PACKAGE_RIGHT" ? "right" : "center", record.code);
      assert.equal(contract.crop_spec, null, record.code);
    }
    const structured = thumbnailStructuredDataImage(surfaces[0], new URL("https://example.test"));
...
test("Phase 5G 8822-9821 appends the exact 596 delegated adaptive approvals", async () => {
  const source = await readFile("data/thumbnail-phase5-reviewed-decisions.csv", "utf8");
  const preservedPrefix = `${source.split("\n").slice(0, 2696).join("\n")}\n`;
  assert.equal(
    crypto.createHash("sha256").update(preservedPrefix).digest("hex"),
    "8628324e7af2791170d3001a858b4d57b444444cac6da2610b02e31f867cea42",
  );
  const batch = GENERATED_PHASE5_REVIEWED_DECISION_RECORDS.filter(
    (record) => record.approval_batch === "phase5g-8822-9821",
  );
  assert.equal(batch.length, 596);
  assert.equal(new Set(batch.map((record) => record.code)).size, 596);
  assert.deepEqual(
    batch.reduce((counts, record) => {
      counts[record.mode] = (counts[record.mode] ?? 0) + 1;
      return counts;
    }, {}),
    { PACKAGE_FULL: 55, PACKAGE_RIGHT: 442, PACKAGE_CENTER: 2, SAMPLE: 97 },
  );
  const objectFit = {
    PACKAGE_FULL: "contain",
    PACKAGE_RIGHT: "cover",
    PACKAGE_CENTER: "cover",
    SAMPLE: "scale-down",
  };
  for (const record of batch) {
    assert.equal(record.approved_by, "owner_delegated_via_chatgpt", record.code);
    ...
    assert.equal(effective?.output_path_or_url, record.output_path_or_url, record.code);
    assert.equal(effective?.output_hash, record.output_hash, record.code);
    if (record.mode === "PACKAGE_RIGHT" || record.mode === "PACKAGE_CENTER") {
      const bytes = await readFile(`public${record.output_path_or_url}`);
      assert.equal(crypto.createHash("sha256").update(bytes).digest("hex"), record.output_hash, record.code);
    } else {
      assert.equal(record.source_path_or_url, record.output_path_or_url, record.code);
...
test("Phase 5G priority cohort appends the exact 165 delegated adaptive approvals", async () => {
  const source = await readFile("data/thumbnail-phase5-reviewed-decisions.csv", "utf8");
  const preservedPrefix = `${source.split("\n").slice(0, 3292).join("\n")}\n`;
  assert.equal(
    crypto.createHash("sha256").update(preservedPrefix).digest("hex"),
    "11dffee231495ac2d77398de4fd1441ba9d55690e8f254f82dc5bbcaf143c42f",
  );
  const batch = GENERATED_PHASE5_REVIEWED_DECISION_RECORDS.filter(
    (record) => record.approval_batch === "phase5g-priority-20260831",
  );
  assert.equal(batch.length, 165);
  assert.equal(new Set(batch.map((record) => record.code)).size, 165);
  assert.deepEqual(
    batch.reduce((counts, record) => {
      counts[record.mode] = (counts[record.mode] ?? 0) + 1;
      return counts;
    }, {}),
    { PACKAGE_RIGHT: 150, PACKAGE_CENTER: 7, PACKAGE_FULL: 7, SAMPLE: 1 },
  );
  const objectFit = {
    PACKAGE_FULL: "contain",
    PACKAGE_RIGHT: "cover",
  ...
  const sample = batch.find((record) => record.code === "MIZD00536");
  assert.equal(sample?.mode, "SAMPLE");
  assert.equal(sample?.source_id, "sample:9");
  assert.equal(PRODUCTION_THUMBNAIL_REGISTRY_CONFLICTS.length, 0);
  assert.equal(PRODUCTION_THUMBNAIL_DECISIONS.size, 3560);
});
...
test("READY decisions use one mode-level object-fit contract", () => {
  ...
  assert.deepEqual(counts, {
    "SAMPLE|scale-down": 416,
    "PACKAGE_RIGHT|cover": 2806,
    "PACKAGE_FULL|contain": 174,
    "SCENE_FULL|contain": 1,
    "SCENE_CROP|scale-down": 29,
    "PACKAGE_CENTER|cover": 134,
  });
});
