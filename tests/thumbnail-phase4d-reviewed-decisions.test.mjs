...
test("the formal fixture contains R2 final decisions and R1 teacher decisions without production inflation", () => {
  ...
  for (const row of fixtures) {
    ...
    if (row.production_effect === "PRODUCTION_DECISION") {
      ...
      assert.equal(decision.mode, row.expected_mode, row.code);
      assert.equal(decision.source_id, row.expected_source_id, row.code);
    } else {
...
test("all 14 changed decisions resolve identically on list detail related recent and JSON-LD", () => {
  for (const record of GENERATED_PHASE4D_REVIEWED_DECISION_RECORDS) {
    ...
    assert.equal(decision.source_path_or_url, record.source_path_or_url, record.code);
    assert.equal(decision.source_hash, record.source_hash, record.code);
    assert.equal(decision.output_path_or_url, record.output_path_or_url, record.code);
    assert.equal(decision.output_hash, record.output_hash, record.code);
    assert.equal(decision.crop_spec, null, record.code);
    const expectedFit = record.mode === "PACKAGE_FULL" ? "contain" : "cover";
    ...
    const structured = thumbnailStructuredDataImage(surfaces[0], "https://preview.example.test");
    const expectedUrl = record.output_path_or_url.startsWith("https://")
      ? record.output_path_or_url
      : `https://preview.example.test${record.output_path_or_url}`;
