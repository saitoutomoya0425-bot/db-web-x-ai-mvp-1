...
const inputPath = path.join(root, "data", "thumbnail-phase4d-reviewed-decisions.csv");
const generatedPath = path.join(
  root,
  "src",
  "lib",
  "thumbnail",
  "generated-phase4d-reviewed-decisions.ts",
);
const EXPECTED_COUNT = 14;
const EXPECTED_APPROVED_BY = "USER_HANDOFF";
const EXPECTED_APPROVED_AT = "2026-08-02";
const EXPECTED_APPROVAL_BATCH = "PHASE_4D_USER_REVIEW";
const SHA256 = /^[a-f0-9]{64}$/;
const MODE_COUNTS = Object.freeze({ SAMPLE: 4, PACKAGE_RIGHT: 8, PACKAGE_FULL: 2 });
...
function canonicalCode(value) {
  const raw = requiredText(value, "PHASE4D:CODE");
  const result = canonicalizeProductCodeValue(raw);
  if (!result.canonical || result.rejected || result.canonical !== raw) {
    throw new Error(`PHASE4D:INVALID_CANONICAL_CODE:${raw}`);
  }
  return raw;
}

function safeDmmUrl(value, context) {
  let parsed;
  try {
...
function expectedExternalPath(code, mode, sourceId) {
  const slug = code.toLowerCase();
  if (mode === "PACKAGE_FULL") {
    if (sourceId !== "dvd:full") throw new Error(`PHASE4D:${code}:INVALID_FULL_SOURCE_ID`);
    return `/digital/video/${slug}/${slug}pl.jpg`;
  }
  const match = /^sample:([1-9]\d*)$/.exec(sourceId);
  if (!match) throw new Error(`PHASE4D:${code}:INVALID_SAMPLE_SOURCE_ID`);
  return `/digital/video/${slug}/${slug}jp-${match[1]}.jpg`;
}

async function verifyLocalSource(code, sourceLocalPath, expectedHash) {
  const absolute = path.resolve(root, sourceLocalPath);
  const relative = path.relative(root, absolute);
  if (!relative || relative.startsWith("..") || path.isAbsolute(relative)) {
    throw new Error(`PHASE4D:${code}:LOCAL_SOURCE_OUTSIDE_REPOSITORY`);
  }
  const info = await fs.lstat(absolute);
  if (!info.isFile() || info.isSymbolicLink()) {
    throw new Error(`PHASE4D:${code}:LOCAL_SOURCE_NOT_REGULAR_FILE`);
  }
  const bytes = await fs.readFile(absolute);
  if (!bytes.length) throw new Error(`PHASE4D:${code}:LOCAL_SOURCE_EMPTY`);
  if (sha256(bytes) !== expectedHash) {
    throw new Error(`PHASE4D:${code}:LOCAL_SOURCE_HASH_MISMATCH`);
  }
}

async function materializeRecord(row) {
  const code = canonicalCode(row.code);
  const context = `PHASE4D:${code}`;
  ...
  await verifyLocalSource(code, sourceLocalPath, sourceHash);
  ...
  if (mode === "PACKAGE_RIGHT") {
    ...
    if (
      ...
      sourceLocalPath !== expectedSource ||
      outputPath !== expectedOutput
    ) {
      throw new Error(`${context}:RIGHT_PATH_CONTRACT_MISMATCH`);
    }
  } else if (mode === "SAMPLE" || mode === "PACKAGE_FULL") {
    const sourceUrl = safeDmmUrl(sourcePath, context);
    const outputUrl = safeDmmUrl(outputPath, context);
    const expectedPath = expectedExternalPath(code, mode, sourceId);
    if (
      sourcePath !== outputPath ||
      sourceUrl.pathname !== expectedPath ||
      outputUrl.pathname !== expectedPath
    ) {
      throw new Error(`${context}:EXTERNAL_PATH_CONTRACT_MISMATCH`);
    }
  } else {
...
function stableSource({ records, inputHash }) {
  return `/* This file is generated by scripts/generate-thumbnail-phase4d-reviewed-decisions.mjs. */
...
export const GENERATED_PHASE4D_REVIEWED_INPUT_SHA256 = ${JSON.stringify(inputHash)};
`;
}

export async function generatePhase4DSource({ decisionFilePath = inputPath } = {}) {
  ...
  if (JSON.stringify(header) !== JSON.stringify(EXPECTED_HEADERS)) {
    throw new Error("PHASE4D:INVALID_HEADERS");
  }
  const rows = parseCsv(input);
  if (rows.length !== EXPECTED_COUNT) {
    throw new Error(`PHASE4D:COUNT_MISMATCH:${rows.length}:${EXPECTED_COUNT}`);
  }
  const records = [];
  const seen = new Set();
  for (const row of rows) {
    const record = await materializeRecord(row);
    if (seen.has(record.code)) throw new Error(`PHASE4D:DUPLICATE_CODE:${record.code}`);
  ...
  if (
    ...
    throw new Error(`PHASE4D:MODE_COUNT_MISMATCH:${JSON.stringify(counts)}`);
  }
...
export async function runCli(args = process.argv.slice(2)) {
  const result = await generatePhase4DSource();
  if (args.includes("--write")) {
    ...
    if (existing !== result.source) {
      throw new Error("PHASE4D_REVIEWED_REGISTRY_OUT_OF_SYNC");
    }
  } else {
