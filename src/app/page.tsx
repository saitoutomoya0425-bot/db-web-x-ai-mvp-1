...
import { SourceAwareWorkCarousel } from "@/components/source-aware-work-carousel";
...
export default async function HomePage() {
  const [newest, popular, recommended, weeklyRanking, actresses, makers] = await Promise.all([
    getSourceAwareCatalogWorks({ sort: "newest", limit: 30 }),
  ...
  const hasWorks = newest.works.length || popular.length || recommended.length || weeklyRanking.length;
  return (
    <main>
      ...
      <SourceAwareWorkCarousel eyebrow={<span className="flex items-center gap-2"><Clock3 className="size-4" />新着</span>} title="新着作品" works={newest.works} action={<Link href="/works?sort=newest" className="inline-flex items-center gap-1 text-sm text-slate-400 hover:text-white">新着を探す<ArrowRight className="size-4" /></Link>} className="pb-6 sm:pb-10" />
