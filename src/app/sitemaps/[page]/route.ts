...
export async function GET(_request: Request, { params }: { params: Promise<{ page: string }> }) {
  ...
  const { data } = await supabase.from("videos").select("product_code,updated_at").order("id").range(start, start + PAGE_SIZE - 1);
  const staticUrls = page === 0 ? ["", "/works", "/makers", "/genres", "/ranking", "/rankings", "/rankings/actress", "/rankings/maker", "/rankings/series"].map((path) => ({ url: `${site}${path}`, modified: new Date().toISOString() })) : [];
