...
export type CatalogSort="popular"|"new"|"newest"|"release"|"recommended";
export async function getCatalogWorks(options:{limit?:number;offset?:number;sort?:CatalogSort;genre?:string;maker?:string}={}){
  const supabase=await createClient(),limit=Math.min(Math.max(options.limit??24,1),1000),offset=Math.max(options.offset??0,0);
  ...
  if(options.sort==="new"||options.sort==="newest")query=query.order("created_at",{ascending:false});
...
export async function getGenreFacets(limit=100,offset=0){
  ...
  if(!error)return data??[];
  const {data:fallback}=await supabase.from("videos").select("genre,popularity").eq("is_published",true).not("genre","is",null).limit(1000);
